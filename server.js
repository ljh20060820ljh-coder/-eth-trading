...
console.log("👑 V7.1 终极雷达修复版 (动态火力+追踪防弹衣+灵魂雷达+长期记忆) 已上线！");
...
function fetchJSON(url) { return new Promise((resolve, reject) => { https.get(url, { headers: { 'User-Agent': 'Assassin-Bot/7.1' } }, (res) => { let data = ''; res.on('data', chunk => data += chunk); res.on('end', () => { try { resolve(JSON.parse(data)); } catch(e) { reject(e); } }); }).on('error', reject); }); }
...
async function runMonitor() {
  ...
  try {
    ...
    if (aiDecision) {
        console.log(`🧠 [${currentStrategy}] 汇报: 方向=${aiDecision.direction}, 把握=${aiDecision.confidence}分, 理由=${aiDecision.reason}`);
    }
...
async function runHourlyReport() {
    ...
    let msg = `<b>📊 V7.1 小时级简报</b><br><br>当前持仓: ${position.status === 'NONE' ? '空仓蹲草丛' : `${position.status} (${position.qty} ETH)`}<br><br><b>近期流水:</b><br>`;
...
【历史教训】：${memoryStr || "暂无经验"}
【今天实战】：${tradesStr}。
任务：结合历史教训复盘今天表现，明天该怎么优化？以"老板你好..."开头，200字内总结。`;
...
http.createServer((req, res) => {
  ...
  if (req.url === '/status') { res.end(JSON.stringify({ status: "alive", mode: "V7.1 Boss Mode", position })); return; }
  res.end("System Running: V7.1 Auto Trade Mode");
